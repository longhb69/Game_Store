...
export default function GameImageVideo(props) {
    ...
    const getResizeImage = (imageUrl) => {
        const part = imageUrl.split('/upload/')
        return part[0] + `/upload/w_142,c_fill/` + part[1]
    }
    return (
        <>
                        ...
                        {props.game_image && props.game_image.length > 0 ? 
                            props.game_image.map((image, index) => (
                                <SwiperSlide style={{width: '200px', borderRadius: '4px'}} key={index}>
                                    ...
                                    >
                                        <img src={getResizeImage(image.image)} className={`rounded ${index===currentSlide-1 ? 'border border-[#fff] opacity-[1]' : 'opacity-[0.6]'}  hover:opacity-[1]  cursor-pointer  max-w-[180px] h-[80px] transition-opacity ease-in-out duration-[450ms]`}></img>
                                    </div>
                                </SwiperSlide>
                            ))
                        : null}

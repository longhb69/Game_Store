            ...
            <div className="flex flex-col pr-5">
                <div className="flex pt-2 justify-end">
                    <div className={`ml-[10px] ${beginSlide ? 'pointer-events-none select-none cursor-default' : ''}`}>
                        <button
                            className={` ${beginSlide ? 'css-172' : 'css-17'} relative w-[31px] h-[31px] flex items-center justify-center`}
                            onClick={() => prev()}>
                            <span className={`lock w-[13px] h-[13px] scale-x-[-1] ${beginSlide ? 'opacity-[0.5] select-none' : ''}`}>
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    class={`w-full h-full ${beginSlide ? 'pointer-events-none select-none' : ' '}`}
                                    viewBox="0 0 5 9">
                    ...
                    <div className={`ml-[10px] ${endSlide ? 'select-none pointer-events-none cursor-default' : ' '}`}>
                        <button
                            className={`${endSlide ? 'css-172' : 'css-17'} relative w-[31px] h-[31px] flex items-center justify-center`}
                            onClick={() => next()}>
                            <span className={`block w-[13px] h-[13px] z-[1] ${endSlide ? 'opacity-[0.5]' : ' '}`}>
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    class={`w-full h-full ${endSlide ? 'pointer-events-none select-none' : ' '}`}
                                    viewBox="0 0 5 9">
                                    <path stroke="currentColor" d="M1 1l3 3.5L1 8" fill="none" fill-rule="evenodd"></path>
                                </svg>
                            </span>
